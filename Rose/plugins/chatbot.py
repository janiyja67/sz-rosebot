...
async def szcbot(_, message: Message):
    ...
    if chat:
        ...
        try:
           ...
           affiliateplus = requests.get(f"https://api.affiliateplus.xyz/api/chatbot?message={text}&botname=Rose&ownername=@invisible_einstein&user={chat_id}")
           textmsg = (affiliateplus.json()["message"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "AL Kuppiya Protector")
           if "Lebyy_Dev" in textmsg:
               textmsg = textmsg.replace("Lebyy_Dev", "Invisible Einstein")
           ...
           trtoen = (message.text if lang=="en" else tr.translate(message.text, dest="en").text).replace(" ", "%20")
           text = trtoen.replace(" ", "%20") if len(message.text) < 2 else trtoen
           safeone = requests.get(f"https://api.safone.tech/chatbot?message={text}&bot_name=alkuppiyaprotector&bot_master=Einstein&user_id={user_id}")
           textmsg = (safeone.json()["answer"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "AL Kuppiya Protector")
           if "[Safone]" in textmsg:
               textmsg = textmsg.replace("[Safone]", "Invisible Einstein")
           msg = tr.translate(textmsg, src='en', dest=lang)
           await message.reply_text(msg.text)
...
**Language Support**
AL Kuppiya Protector AI chatbot support almost all languages in world .
